...
const ButtonsBlock = ({onClick}: {onClick: (dir: 'prev' | 'next') => void}) => {
  return (
    <div className={cn(styles.navigation, screen.currentScreen === 0 && styles.navigation_disclaimer)}>
...
const Navigation: FC = observer(() => {
  ...
  if (screen.currentScreen === 0) return <ButtonsBlock onClick={onClick}/>

  if (!mounted) return null

  return (
    <Portal>
      <ButtonsBlock onClick={onClick}/>
    </Portal>
  )
})
